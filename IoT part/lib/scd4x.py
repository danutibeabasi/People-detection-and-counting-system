...
class SCD4X:
    ...
    **Quickstart: Importing and using the SCD4X**
    ...
    def __init__(self, i2c : machine.I2C, address=SCD4X_DEFAULT_ADDR):
        self._i2c = i2c
...
#                raise RuntimeError(f"CRC check failed while reading data: { self._crc_buffer } -> { self._crc8(self._crc_buffer) } neq { buf[i + 2] }" )
        return True
    ...
    def _read_reply(self, num):
        self._buffer[0:num] = self._i2c.readfrom(self._address, num)
